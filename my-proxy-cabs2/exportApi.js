...
const fs         = require('fs'),
      ...
      getopt     = new Getopt(common.commonOptions.concat([
        ...
        ['R' , 'revision=ARG', 'revision of the asset to export. Default: latest'],
         ['O' , 'organization=ARG', 'organization where to deploy. Mandatory']
      ])).bindHelp();
...
function unzipExportedFile(result) {
  console.log("Unzipping the result file"+result);
  try {

          var zip = new admZip(path.join(opt.options.destination, result));
          console.log('start unzip of file:'+result+ " at path:-> "+opt.options.destination);
          zip.extractAllTo(opt.options.destination,true);
          console.log('finished unzip, unlinking the zip now');

          fs.unlinkSync(path.join(opt.options.destination, result));

          console.log('zip file has been removed');

          //extract(filename,{ dir: cwd});
          //console.log('Extraction complete');

        } catch(err){

        console.log('Error encoutered during extraction.'+err);
...
apigeeEdge.connect(common.optToOptions(opt), function(e, org) {
  ...
  if (opt.options.name && opt.options.revision) {
    ...
    fileExportName=exportLatestRevisionOfProxy(org, opt.options.name, function(e, result){
      common.logWrite('ok');
      //return result;
      unzipExportedFile(result);
    });
...
//once export has been done we can unzip the file